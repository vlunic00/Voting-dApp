...
export default function Home() {
    // Consume the global state from the combined context
    const { contract, signer, votingState, votingDuration } = useContext(Web3Context);

    // State specific to the homepage
    ...
    // Effect to check chairperson status (for showing admin links)
    useEffect(() => {
    ...
    // This boolean will control if the candidate buttons are clickable
    const isVotingCurrentlyActive = votingState === 'active' && !isTimeUp;

    // A helper to render the correct status component (message, timer, or results)
    const renderStatusComponent = () => {
        // Determine the final state, accounting for the local timer
        ...
        switch (finalState) {
            ...
            case 'finished':
                // Pass a duration of 0 to show the final result instantly
    ...
    return (
        <div className="flex flex-col pt-[10vh] items-center px-4">
            ...
            <div className="mt-10 w-full flex flex-col items-center">
                ...
                {/* Render the status/timer/result component on top */}
                {renderStatusComponent()}

                {/* Always render the candidate list below, passing the active state */}
