...
import { ContractContext, SignerContext } from '@/contexts/Web3Context'; // Make sure this path is correct
...
const handleConfirmVote = async () => {
    ...
    try {
        // 2. Call the contract's 'vote' function
        console.log(`Sending vote for candidate index: ${selectedCandidate.index}`);
        ...
        if (reason.includes("Has no right to vote")) {
            reason = "You do not have the right to vote. Please contact the chairperson.";
    ...
    if (!candidates || candidates.length === 0) {
        return (
            <div className="p-8 border-2 border-slate-600 rounded-2xl">
                <p className="py-2">No candidates available, or still loading...</p>
            </div>
        );
    }
    ...
    return (
        <div className="p-8 border-2 border-slate-600 rounded-2xl w-full max-w-lg">
            <h2 className="text-2xl font-bold mb-4 text-center">Vote for a Candidate</h2>
